...
class UserModel():
    ...
    def get_user(self, id):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    """SELECT id, first_name, last_name, email FROM public.users where id = %s """, (id,))
                ...
                if result != None:
                    response = UserEdit(
                        result[0], result[1], result[2], result[3]).to_JSON()
    ...
    def login_user(self, email, password):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    """SELECT password, id, id_rol FROM public.users where email = %s """, (email,))
                result = cursor.fetchone()
    ...
    def get_all_users_from_admin(self):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    """ SELECT u.id, u.first_name, u.last_name, u.email,  r.rol_name
	                    FROM public.users u
	                    INNER JOIN public."Rols" r ON u.id_rol= r.id
                        WHERE u.id_rol = 1; """)
